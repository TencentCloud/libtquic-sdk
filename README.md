...
将libtquic.so,tnet_quic_request.h,tquic_demo_bin.cc放在同一目录下
...
将android静态库.a 以及 头文件 tnet_quic_request.h放入android studio，编写cmakelist即可

#### 如何基于tquic编写代码

接口简便，参见tquic_demo_bin.cc即可，有详细注释
...
android 参见demo/android

```C++
#include "tencent/src/tnet_quic_request.h"  // 导入API头文件
...
class DemoDelegate : public TnetRequestDelegate {
 public:
  ...
  void OnRequestFinish(int stream_error) {
    if (stream_error == 0) {
      // 请求正常结束
      request_ret = FINISH;
    } else {
      // 请求异常结束
      request_ret = FAIL;
      cout << "Request finish with error: " << stream_error;
    }
    // 唤醒业务线程
    s_cv.notify_all();
  }
